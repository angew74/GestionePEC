...
namespace Com.Delta.Print.Engine
{
	...
	public sealed class ReportDocument : System.Drawing.Printing.PrintDocument
	{
		...
		/// See <see cref="Com.Delta.Print.Engine.ChartBox.AddSerie">Com.Delta.Print.Engine.ChartBox.AddSerie</see> for an example
		...
		public void AddUserPaint(string userPaintName, Com.Delta.Print.Engine.UserPaint userPaint, bool overrideTemplateSize)
		{
		...
		/// <param name="userPaintName">The name of the UserPaintElement to set data.</param>
		/// <param name="userPaint">UserPaint object to display.</param>
		public void AddUserPaint(string userPaintName, Com.Delta.Print.Engine.UserPaint userPaint)
		{
		...
		/// <param name="documentRoot">Report Document Root (<see cref="Com.Delta.Print.Engine.ReportDocument.DocRoot"/>) folder</param>
		/// <remarks>
        ...
        public byte[] SerializeToHtmlStream()
        {
            HtmlSerializer htmlSerializer = new HtmlSerializer();
            return htmlSerializer.SerializeToHTML(this);
        }

        public string SerializeToHtmlString()
        {
            HtmlSerializer htmlSerializer = new HtmlSerializer();
            return htmlSerializer.SerializeToHTMLString(this);
        }
		...
		/// <summary>
		/// Gets or sets the 
		/// <see cref="Com.Delta.Print.Engine.ReportDocument.LayoutType">Com.Delta.Print.Engine.ReportDocument.LayoutType</see>
		...
		/// <summary>
		/// Gets or sets the 
		/// <see cref="Com.Delta.Print.Engine.Paper.Type">Com.Delta.Print.Engine.Paper.Type</see>
		/// for the ReportDocument.
		/// </summary>
		...
		internal Section[] GetSections(string name)
		{
			...
			return (Section[])list.ToArray(typeof(Com.Delta.Print.Engine.Section));
		}
		...
		private void resolveContents(XmlNode theNode)
		{
			...
			sections.Clear();
            // Ciro Cardone 06.11.2014 - Ho aggiunto in ingresso al metodo i parametri false(designMode) e ""(removeParameters)
            // bisogna controllare se questi due parametri sono necessari quando si tratta di un Content oppure 
            // possono essere lasciati vuoti
			sections.Add(new Section(this, theNode, true, ""));
		}
		...
		private void resolveSections(XmlNode theNode)
		{
            ...
            // Ciro Cardone 06.11.2014 - BEGIN
            // Prendo i valori settati nel parametro ROMOVE (i paramentri devono essere separati dal carattere ',')
            string removeParameters = "";
            if ( !String.IsNullOrEmpty((string)defaultParameterValues["Remove"]) )
                removeParameters = (string)defaultParameterValues["Remove"];
            // Ciro Cardone 06.11.2014 - END

			for (int i=0;i<childNodes.Count;i++)
			{
                new Section(this, childNodes[i], designMode, removeParameters);
			}
		}
