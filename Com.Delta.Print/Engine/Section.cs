...
namespace Com.Delta.Print.Engine
{
    ...
    public sealed class Section
    {
        ...
        // Ciro Cardone - Begin 15.10.2014
        //public List<String> theFlowElements = new List<String>();
        //Ciro Cardone - End 15.10.2014
        ...
        /// Creates new instance of section.
        /// </summary>
        internal Section(ReportDocument document, XmlNode node, bool designMode, string removeParameters)
            : this(document)
        {
            ...
            // Ciro Cardone 31.10.2014 - Begin

            List<String> elimina = new List<String>();
            Hashtable propCancI = new Hashtable();
            ICustomPaint[] oggettiTemporanei = new ICustomPaint[0];
            string delElem = "";

            if (!designMode)
            {
                if (!removeParameters.Equals("") )
                {
                    string[] app = removeParameters.Split(',');
                    foreach (string ap in app)
                        elimina.Add(ap);
                }
                if (elimina.Count != 0)
                {
                    ...
                    {
                        bool trovato = false;
                        int indiceC = 0; // Indice dell'elemento da cancellare impostato a 0
                        foreach (String bla in elimina)
                        {
                            if (bla.Equals(objects[ii].Name))
                            {
                                trovato = true;
                                indiceC = ii;
                            }
                        ...
                        if (!trovato)
                        {
                            tmp.AddRange(oggettiTemporanei);
                            tmp.Add(objects[ii]);

                        }
                        else
                        {
                            propCancI.Add(Chain.IndexOf(indiceC), Chain[Chain.IndexOf(indiceC)]);
                            Chain[Chain.IndexOf(indiceC)] = -1; // Settando il valore a -1 non verrà valutato nella generazione della chiave
                        }
                    }

                    bool successivo = false;
                    int z = 0;
                    foreach (int ap in Chain)
                    {
                        if (ap == -1)
                        {
                            if (successivo == false)
                        ...
                        else { successivo = false; z++; }
                    }

                    string[] split = delElem.Split('-');

                    foreach (string ap in split)
                    {
                        string[] tokens = ap.Split(';');
                        int[] convertedItems = Array.ConvertAll<string, int>(tokens, int.Parse);
                        int min = convertedItems[0];
                        int max = convertedItems[convertedItems.Length - 1];
                        ICustomPaint objMin = null;
                        ICustomPaint objMax = null;
                        ICustomPaint objSuccessivo = null;
                        ICustomPaint objPrecedente = null;
                        foreach (DictionaryEntry elemento in propCancI)
                        {
                            if ((int)elemento.Key == min)
                                objMin = objects[(int)elemento.Value];
                            if ((int)elemento.Key == max)
                                objMax = objects[(int)elemento.Value];
                        }
                        objSuccessivo = objects[(int)Chain[max + 1]];
                        objPrecedente = objects[(int)Chain[min - 1]];
                        int diff = objSuccessivo.Y - (objMax.Y + objMax.Height);
                        int diff1 = objPrecedente.Y + objPrecedente.Height;
                        int nuovaY = diff1 + diff;
                        int diff2 = objSuccessivo.Y - nuovaY;
                        foreach (int index in Chain)
                            if (index >= 0 && index < objects.Length)
                                if (objects[index].Y > objMin.Y)
                                    objects[index].Y = objects[index].Y - diff2;
                        //foreach (ICustomPaint objAppo in objects)
                        //{
                        //    if (objAppo.Y > objMin.Y)
                        //        objAppo.Y = objAppo.Y - diff2;
                        //}
                        objSuccessivo.Y = nuovaY;
                    }
                    oggettiTemporanei = (ICustomPaint[])tmp.ToArray(typeof(ICustomPaint));
                }
            }
            // Ciro Cardone 31.10.2014 - End

            foreach (int index in Chain)
                if (index >= 0 && index < objects.Length)
                    t.Add(objects[index]);
            
            // Ciro Cardone 03.11.2014 - Begin
            if (oggettiTemporanei.Length != 0)              
                objects = oggettiTemporanei;
            // Ciro Cardone 03.11.2014 - End
        ...
        /// <summary>
        /// Gets a collection of <see cref="Com.Delta.Print.Engine.ICustomPaint">Com.Delta.Print.Engine.ICustomPaint</see> objects.
        /// </summary>
        [Browsable(false)]
                ...
                switch (childNodes[i].Name)
                {
                    case "text":
                        ...
                        try
                        {
                            ...
                            textField.TextAlignment = (TextField.TextAlignmentType)Enum.Parse(typeof(Com.Delta.Print.Engine.TextField.TextAlignmentType), alignmentType, false);
                        }
                        catch (Exception) { }
        ...
        private TimelinePeriod[] resolveTimelinePeriods(XmlNode theNode)
        {
            ...
            return (TimelinePeriod[])tmp.ToArray(typeof(Com.Delta.Print.Engine.TimelinePeriod));
        ...
        private TimelinePoint[] resolveTimelinePoints(XmlNode theNode)
        {
            ...
            return (TimelinePoint[])tmp.ToArray(typeof(Com.Delta.Print.Engine.TimelinePoint));
        }

        private Com.Delta.Print.Engine.Map.MapPoint[] resolveMapPoints(XmlNode theNode)
        {
            ...
            for (int i = 0; i < childNodes.Count; i++)
            {
                Com.Delta.Print.Engine.Map.MapPoint point = new Com.Delta.Print.Engine.Map.MapPoint();
            ...
            return (Com.Delta.Print.Engine.Map.MapPoint[])tmp.ToArray(typeof(Com.Delta.Print.Engine.Map.MapPoint));
        }
       ...
       */
        ...
        public object getChainElement(int i)
        {
        ...
        /// </summary>
        /// <param name="chartName"><see cref="Com.Delta.Print.Engine.ChartBox">Com.Delta.Print.Engine.ChartBox</see> to set categories for.</param>
