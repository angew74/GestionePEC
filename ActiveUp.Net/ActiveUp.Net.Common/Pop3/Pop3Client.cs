    ...
    public class Pop3Client : System.Net.Sockets.TcpClient
    {
        ...
        #region Connecting, authenticating and disconnecting
            ...
            public string Disconnect()
            {
                ...
                if(response.ToUpper().Contains("Command \"QUIT\" failed"))
                {
                    Thread.Sleep(5000);                    
                    response = this.Command("QUIT"); 
                }
                this.Close();
        ...
        public string CommandMultiline(string command)
        {
            ...
            if (!str.StartsWith("+")) return( null ); //LR//
            while (true)
            {
                str = sr.ReadLine();
            ...
            if (messageIndex > 0 && messageIndex <= this.MessageCount)
            {
                ...
                if (str.StartsWith("+OK"))
                {
                    while (true)
                    {
                        ...
                        if (str != ".")
                        {
                            //aggiunta da Luigi per gestire il "dot-stuffing"
                            if (str.StartsWith("..") && str.Length > 2) str = str.Substring(1);
                            if (str != "..") sb.Append(str + "\r\n");
                            else sb.Append("." + "\r\n");
                        }
                        else break;
                ...
                if ( sb.Length > 0 )
                    {
#if !PocketPC
                    buf = System.Text.Encoding.GetEncoding("iso-8859-1").GetBytes(sb.ToString());
#else
                    buf = PPCEncode.GetBytes(sb.ToString());
#endif
                    }
        ...
        {
            byte [] buffer;
            if ( ( buffer = this.RetrieveMessage(messageIndex) ) == null )
                return( null );
              else
                return ActiveUp.Net.Mail.Parser.ParseMessage( buffer );
        }
        ...
        public byte[] RetrieveHeader(int messageIndex, int numberOfBodyLines)
        {
            ...
            if ( header != null )
                {
#if !PocketPC
            buf = System.Text.Encoding.GetEncoding("iso-8859-1").GetBytes(header);
#else
            buf = PPCEncode.GetBytes(header);
#endif
                }
        ...
        {
            byte [] buffer;
            if ( ( buffer = this.RetrieveHeader(messageIndex) ) == null )
                return( null );
            return ActiveUp.Net.Mail.Parser.ParseHeader( buffer );
        }
        ...
        public System.Collections.Generic.List<PopServerUniqueId> GetUniqueIds()
        {
            ...
            foreach (string line in lines)
            {
               ...
               // string un = pse.UniqueId.Replace(',', '§');
               // pse.UniqueId = un;
                uids.Add(pse);
            }
