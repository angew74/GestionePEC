...
namespace Com.Delta.PrintManager.Engine.Pdf
{
	...
	public class PdfCellRange
	{
		...
		internal PdfArea Area
		{
			get
			{
				return this.owner.Cell(startRow,startColumn).Area.Merge(this.owner.Cell(endRow,endColumn).Area);
			}
		}
		/// <summary>
		...
		{
			foreach (PdfCell rc in this.Cells)
			{
				rc.horizontalAlignment = horAlignment;
				rc.verticalAlignment = verAlignment;
			}
		}
